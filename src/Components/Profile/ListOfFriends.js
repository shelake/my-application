...
function MyVerticallyCenteredModal(props) {
    ...
    useEffect(() => {
        axios.get(`http://localhost:8086/api/v1/users/${currentUser.userid}/friends`)
            .then(response => {
...
const ListOfFriends = () => {
    ...
    useEffect(() => {
        axios.get(`http://localhost:8086/api/v1/users/${currentUser.userid}/friends`) // Assuming user ID is 13
            .then(response => {
                setUserFriends(response.data);
            })
            .catch(error => {
