...
function GroupPage() {
  ...
  const fetchGroups = async () => {
    try {
      const response = await axios.get(`http://localhost:8086/api/v1/users/${currentUser.userid}/groups`);
  ...
  const handleAddGroup = async () => {
    try {
      await axios.post(`http://localhost:8086/api/v1/groups/${newGroupName}/${currentUser.userid}`);
      fetchGroups();
      setShowModal(false);
      setNewGroupName('');
    } catch (error) {
  ...
  const handleRemoveGroup = async (groupId) => {
    try {
      await axios.delete(`http://localhost:8086/api/v1/groups/delete/${groupId}`);
      fetchGroups();
    } catch (error) {
